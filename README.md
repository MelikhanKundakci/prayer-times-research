...
The app's primary direction is now **local times at the user's point under documented rules**. The new [local point core](core/local/) separates continuously calculated solar events from named prayer rules and their margins. Its first profile applies published Diyanet criteria and a [declared annual guard for ordinary northern twilight](core/local/NORTHERN-ORDINARY.md), with explicit gaps in summer seasonal policy. A separate opt-in [local seasonal profile](core/local/SEASONAL.md) supplies northern summer estimates under a fully stated project policy where all annual horizon intervals exceed five hours; its added conventions are not presented as Diyanet’s recovered algorithm. The [shared profile registry](core/local/PROFILES.md) also exposes source-backed Egyptian and FCNA angle rules and a bounded Kemenag worked-example adaptation, with prayer starts distinguished from geometry markers. Existing city-calendar reconstructions remain available as a separate research and compatibility track; agreement with a city clock is not the sole measure of a point calculation's correctness.
