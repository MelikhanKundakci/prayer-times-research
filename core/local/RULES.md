...
This page specifies the local Diyanet-criteria point profiles. They combine selected published Diyanet event criteria with independently selected point-astronomy providers and explicit software conventions. They are not Diyanet production algorithms, official local timetables, religious rulings, or notification approval.
...
## Optional SPA point profile

`diyanet-published-spa-point-v1` is an optional second astronomy provider for the same selected ordinary Diyanet criteria. It uses the SPA point provider for both daily event roots and every padded year used by the northern ordinary guard. It retains the exact criteria and margins in the table above: Fajr −18°; sunrise and sunset horizon markers at the same −50′ model threshold; Dhuhr upper transit; factor-one Asr; and margins Fajr 0, sunrise −7, Dhuhr +5, Asr +4, Maghrib +7, Isha 0 minutes. At the northern gate, the same ordinary-only guard may select real −18° Fajr and −16° Isha events only when all its strict daily and annual checks pass. Missing summer events and unresolved transition cases stay blocked.

This provider option does not change the original `diyanet-published-point-v1` profile or the separate `local-northern-seasonal-v1` policy. The two original profiles retain their USNO point model. The SPA profile's rule identifiers carry its own profile ID; its northern annual-context cache key also includes the astronomy provider so it cannot reuse a USNO context. The SPA implementation's time-scale, horizon and astronomical limitations are documented in [SPA-POINT.md](../astronomy/SPA-POINT.md). Using a more detailed provider is not evidence that its times are closer to Diyanet's unpublished production output or observed onset.

The historical Diyanet calendar reconstruction is separate from this point profile. Its low-latitude daily hypothesis uses the existing USNO UTC-00 carrier ([reconstruction guide](../../methods/diyanet/README.md)); it is not the SPA local calculation path.
...
## Northern policy of the two point profiles is intentionally incomplete
...
The public [Diyanet rule evidence note](../../methods/diyanet/RULE-EVIDENCE.md) summarizes the archived criteria and their provenance. The point API may select a bounded subset of raw northern events only when the explicit annual guard in [NORTHERN-ORDINARY.md](NORTHERN-ORDINARY.md) succeeds. The USNO profile uses `diyanet-published-point-v1.fajr.northern-ordinary-18.annual-guard` / `.isha.northern-ordinary-16.annual-guard`; the SPA profile uses the corresponding `diyanet-published-spa-point-v1` identifiers. This guard is a conservative local rule with named night and time-frame conventions; it is not the missing Diyanet seasonal algorithm. If an event is absent, near or inside the unresolved seasonal-transition region, or fails its daily strict comparison, leave its selected value null with `northern-seasonal-policy-not-implemented`. If the annual inputs or horizon checks prevent computing the guard, keep the event blocked and expose the separate annual-context failure reason. Raw −18°/−16° crossings remain under `astronomy.events` for diagnosis; they are not selected unless the guard succeeds. No missing-summer Fajr/Isha estimate is emitted.

The same five-hour day/night rule affects selected sunrise and Maghrib at or above this gate in both point providers. If a required geometric crossing is missing, or the current day or relevant actual adjacent night is at most five hours, checked both before and after margins, the selected horizon event is **`policy-blocked`** under the selected Diyanet profile's `<event>.five-hour-horizon-policy` rule until that estimate rule is implemented. Do not substitute a generic polar-day/polar-night estimate under a Diyanet label. The high-latitude Asr criterion permits Dhuhr to count as Asr when the Asr sign is absent. A proven north-polar-winter case may return that as **`estimated`** under the selected Diyanet profile's `.asr.no-daylight-shadow.use-dhuhr` rule, using Dhuhr's selected instant and its +5-minute adjustment rather than applying an extra Asr +4; the result should carry an explicit no-daylight-shadow reason. Other missing northern Asr cases remain blocked under the profile's `.asr.northern-substitution-policy`. Multiple roots, failed bracketing, or numerical failure are not proof of physical absence and must remain blocked/unavailable, not silently substituted.
