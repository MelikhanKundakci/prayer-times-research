...
export function solarUSNODay(epoch,latitude,longitude){
 ...
 const asrAltitude=factor=>Math.atan(1/(factor+Math.tan(Math.abs(latitude-asrDeclination)*rad)))/rad;
 const asr=factor=>event(asrAltitude(factor),true,13);
 const date=new Date(epoch); // UTC carrier only; consumers must read UTC components.

 return{date,transit:epoch+transit*3600000,sunrise:event(-.833,false,6),sunset:event(-.833,true,18),fajr:event(-18,false,5),isha:event(-18,true,18),asr_standard:asr(1),asr_hanafi:asr(2),declination:noon.declination,asrAltitudes:{asr_standard:asrAltitude(1),asr_hanafi:asrAltitude(2)}};
}
