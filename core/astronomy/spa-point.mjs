 ...
 * Coefficient tables in spa-coefficients.json are extracted from pvlib-python
 * v0.13.1 under its BSD-3-Clause license; see LICENSE-pvlib-SPA and
 * SPA-POINT.md. No pvlib calculation code is copied here.
 */
import coeff from './spa-coefficients.json' with { type: 'json' };

const DEG = Math.PI / 180;
const norm = (x, period = 360) => ((x % period) + period) % period;
const signedAngle = x => norm(x + 180) - 180;
 ...
 *
 * Convention: UT1 = UTC (ΔUT1 is set to zero), hence JDE = JD(UTC) + ΔT/86400.
 * The fixed default is an explicit local-model assumption, not a leap-second
 * prediction or institutional convention. The high-level point adapter below
 * fixes this value; this lower-level function exposes it for sensitivity checks.
 */
export function apparentGeocentricCoordinatesSPA(jdUtc, { deltaTSeconds = 69.184 } = {}) {
  ...
  // GAST, using UT1=UTC. Unlike SPA's conventional mean-longitude EOT,
  // this sidereal angle directly defines the equatorial local hour angle.
  const meanSiderealTime = norm(280.46061837 + 360.98564736629 * (jdUtc - 2451545)
    + 0.000387933 * jc ** 2 - jc ** 3 / 38710000);
  const apparentSiderealTime = norm(meanSiderealTime + nut.longitude * Math.cos(eps));
...
export const SPA_POINT_CONVENTIONS = Object.freeze({
  id: 'SPA-geocentric-continuous-point-v1',
  coordinates: 'Reda–Andreas SPA apparent geocentric solar coordinates',
  ...
  hourAngle: 'GAST + east-positive longitude − apparent geocentric right ascension',
  equationOfTimeMeaning: 'effective UTC-phase adapter derived from GAST−RA, not SPA mean-longitude EOT',
  topocentricParallax: false,
  atmosphericRefractionApplied: false,
  observerElevationMetres: null,
  terrain: null,
});

/**
 * Coordinate interface compatible with solarCoordinatesUSNO.
 * Declination is degrees, right ascension and effective EOT are hours.
 * UTC phase plus effective EOT reproduces GAST−RA modulo one full rotation.
 */
export function solarCoordinatesSPA(julianDate) {
  ...
  const effectiveEquationOfTimeDegrees = signedAngle(
    solar.apparentSiderealTime - solar.rightAscension - utcPhaseDegrees + 180,
  );
  return {
    declination: solar.declination,
    rightAscension: solar.rightAscension / 15,
    equationOfTimeHours: effectiveEquationOfTimeDegrees / 15,
    apparentSiderealTimeDegrees: solar.apparentSiderealTime,
    rightAscensionDegrees: solar.rightAscension,
    conventionalEquationOfTimeMinutes: solar.equationOfTime,
    effectiveEquationOfTimeDegrees,
    ...SPA_POINT_CONVENTIONS,
  };
}
