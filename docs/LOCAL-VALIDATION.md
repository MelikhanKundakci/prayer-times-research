...
The [shared conformance suite](../tests/local-profile-conformance.test.mjs) checks every registered local profile for UTC/local-date agreement, precision, role, availability and ordering over 195 combinations. Coverage distinguishes six populated output fields from five actual prayer-start models. The Egypt/FCNA angle profiles must not promote their auxiliary geometry markers to prayer starts. Kemenag seconds display must remain null for minute-defined selections. Unknown inputs, mutations of profile listings, and attempts to obtain calendar/network data are covered separately.
