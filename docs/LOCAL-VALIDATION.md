...
The [shared conformance suite](../tests/local-profile-conformance.test.mjs) checks all 23 registered local profiles in version 0.6.0 for UTC/local-date agreement, precision, role, availability and ordering over 790 combinations. Coverage distinguishes six populated output fields from five actual prayer-start models. The original Egypt/FCNA angle profiles must not promote their auxiliary geometry markers to prayer starts. The complete software compositions and the opt-in Diyanet SPA point profile have separate names and source scopes. Kemenag seconds display must remain null for minute-defined selections. Unknown inputs, mutations of profile listings, and attempts to obtain calendar/network data are covered separately.
