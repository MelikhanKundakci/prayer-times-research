...
A separate, [conditional reading of the publisher's numerical Temkin table](../methods/turkiye-takvimi/TABLE-COMPONENT.md) treats its historical Istanbul 267 m interpolation as a calculated component and then adds the separately mentioned two-minute precaution. On the **already known complete Istanbul 2026 year**, that gives **1,423/2,190 exact** and **1,830 within one minute**, versus zero and 1,020 for the exported fixed-600-second baseline. Eleven values regress relative to the previous variable-Temkin experiment; sunrise and Maghrib remain systematically wrong. The table does not say whether the caution is already included, the book states an inclusive ten-minute Istanbul mean, and the present production terrain input is unknown. This is a strong diagnostic lead, not a confirmed recipe or default change. A frozen 2027 forecast has no acquired reference rows.
...
The [row-midnight offset diagnostic](../methods/shia-angles/DST-DIAGNOSTIC.md) removes those twelve New York hour-scale differences, but applying it to all older calendars creates **24 new sixty-minute errors in Sydney and Auckland**. It was rejected as a general timezone rule; the published compatibility and physical calculators still use event-time IANA offsets.

A different [UTC-date display hypothesis](../methods/shia-angles/UTC-DATE-DISPLAY.md) retrospectively removes the 36 known hour-scale clock differences across eleven annuals without exact regressions. A later live-interface spot check of 2024 New York DST dates finds eleven of twelve clocks exact and the last within one minute under that same display rule; Kiritimati leap/day-boundary spot checks show no demonstrated date error. ARC's annual response lacks event UTC timestamps, offsets and event dates, so this is not a confirmed source rule or a safe notification-time correction. The physical event instants remain unchanged.
...
The new opt-in [own-ray implementation](../methods/uae-awqaf/OWN-RAY.md) now supplies that independent JavaScript calculation. Across four already exposed, nonoverlapping cohorts it improves **6,840 → 7,239 exact / 9,684**, with **all 398 two-minute differences removed**. It corrects 883 values and regresses 484; **2,445 one-minute differences remain**. It reproduces the earlier PAL candidate’s rounded outputs while using neither its code nor runtime. A separate radial-coordinate implementation checks the integral. Those cohorts were development data for own-ray.

A [new pre-frozen February 2027 transfer](../methods/uae-awqaf/FEBRUARY-2027-TRANSFER.md) compares both unchanged recipes with 504 previously unseen official-PDF fields in the same three mapped regions. V2 gives **353 exact, 487 within one minute, maximum two**; own-ray gives **375 exact, all 504 within one, maximum one**. Every city/event group is non-worse in exact and within-one coverage, so the predeclared numerical gate passes. It is still only one new month and no new geography or confirmed production-point identity. The public default remains V2 pending broader validation.

The next UAE evidence needs are confirmed production point/horizon settings and genuinely new regions and seasons. A varying solar disk/parallax follow-up gained twelve exact values overall but worsened two city/sunrise groups and was rejected. Changing atmospheric coefficients to fit calendar minutes would not establish the publisher’s actual method.
...
Bayynat's original three-month differences are all model-late. A subsequent [uniform nearest-rounding counterexperiment](../methods/bayynat/NEAREST-MINUTE.md) improves known exact agreement to 482/546, but its three fresh point-months give 454/552 exact versus ceiling's 299 while worsening 62 cells and introducing three two-minute Tokyo Fajr differences. It is rejected as a general replacement; the ceiling default remains unchanged. A subsequent [event-time solar candidate](../methods/bayynat/CONTINUOUS.md) improves six known point-months to 1,090/1,098 exact, all within one minute, with five regressions against ceiling. A separate adaptive within-range Buenos Aires alias probe gives 177/180 exact versus 110 with no regressions. The original failed/out-of-range acquisition studies and Berlin/Sydney timezone conflicts remain separate and visible. MUIS's 20 two-minute cases in known 2024–2026 data are concentrated in Maghrib in March 2025 and Asr in September 2025. A [frozen prospective MUIS 2027 comparison](../methods/muis-singapore/PROSPECTIVE-2027.md) gives 1,531/2,190 exact, 2,180 within one minute, and ten two-minute Maghrib differences on 8–17 February. A predeclared one-minute Ramadan Maghrib diagnostic gives 1,551 exact, all 2,190 within one minute and no regressions in that new year. A subsequent [general-rule review](../methods/muis-singapore/MAGHRIB-RESIDUAL-REVIEW.md) rejects an all-date +1 minute Maghrib shift: it reduces 2024–2026 exact values and worsens more-than-one-minute errors. The Ramadan-only offset has no source-backed, offline future-year trigger; the default is unchanged.

The [Roseville-specific experiment](../methods/fcna/) already raises its **paired** two-month Fajr/Isha result from 70/122 to 90/122 exact, with all 122 within one minute in both variants. This is a local publisher experiment, not a revised general FCNA rule. Its development Fajr score and a separately year-ambiguous cohort also regress. A [transferability review](../methods/fcna/TRANSFERABILITY.md) finds all 68 two-minute residuals in the method-confirmed 2025 Roseville sample belong to Isha, while Fajr is entirely within one minute. FCNA's public recommendation specifies angles, not a complete clock-generation algorithm; a frozen 2027 forecast has no qualifying source timetable yet. The generic row above must not conceal the local progress or turn it into a universal guarantee.
...
- **Diyanet northern Isha:** the primary four-city comparison reaches 1,441 minutes because source row date and after-midnight event date are unresolved in 39 comparisons. Under the separately stated next-day-Isha interpretation, all 8,758 comparable values are within one minute. Two original `00:00` fields remain ambiguous. The [source-page check](../methods/diyanet/EVENT-DATE-SEMANTICS.md) finds ordinary clock cells without a zero sentinel or per-event date contract. The conditional interpretation does not establish the provider's intended date.
- **Moonsighting Committee:** the known regression sample retains 28 one-hour disagreements in Sydney/Wellington, while other subsequently acquired city-years match exactly. [Source-row inspection](../methods/moonsighting-committee/DST-SOURCE-DISCREPANCIES.md) places all 28 on the Saturdays before legally defined time changes; changing the runtime to match those rows would give the wrong legal local offset. A good aggregate must not conceal those date-specific failures. Kiritimati additionally has a separately documented source-row hypothesis.
...
The later [Diyanet astronomical-horizon night-quotient test](../methods/diyanet/research/raw-ratio-horizons-2026-09-25b.md) increases retrospective exact agreement by 197 across 61,312 comparable values but loses five within-one-minute results and introduces five three-minute Narvik Fajr errors. It fails its predeclared replacement test; the default is unchanged.
