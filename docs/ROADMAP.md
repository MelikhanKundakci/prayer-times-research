...
The current path independently validates raw solar events and selected summer arithmetic, while continuing to preserve the boundary between raw astronomy and selected prayer events, and keep unsupported cases explicit. In the original profile, at 44.5° north and above, a [conservative annual guard](../core/local/NORTHERN-ORDINARY.md) can admit ordinary real Fajr/Isha; events requiring unresolved seasonal or five-hour replacement rules remain blocked; only the documented northern no-daylight-shadow Asr substitute may be estimated. Do not fill these gaps with generic polar estimates or mirror northern rules into the south. GPS provides coordinates, while the caller must supply an appropriate IANA time zone; elevation, terrain, skyline, and topocentric parallax are outside the current point model.
...
The shared local registry now includes Egyptian and FCNA angle profiles plus a bounded Kemenag worked-example point adaptation. Egypt/FCNA still require explicitly sourced or caller-selected remaining prayer rules before five prayer starts can be emitted under those profiles. Future interval, legal-window and polar policies should fit explicit event semantics rather than forcing every method into an angle preset. A higher-precision independent astronomical reference, height/horizon inputs, and further method migrations remain distinct future steps; the 0.4.0 refactor does not claim to have completed them.
