...
The core's [civil-date default](../../core/diyanet/CIVIL-DATE.md) reproduces the existing date-line variants across all three routes. A complete 59-case replay retains 1,468 additional exact matches in the two affected Pacific annuals, alongside the 52 individual one-minute regressions. This adopts an already studied improvement; it is not a new source acquisition or worldwide accuracy claim.

A subsequent [equation-of-center component test](EQUATION-OF-CENTER.md) isolates one unfitted NOAA orbital correction inside the retained USNO model. On the same 59 annuals it gains 29 exact matches under the conditional evening-cycle interpretation, but creates four additional errors exceeding one minute and loses exact matches in 116 city/event groups. The replacement is rejected. A secondary noon-only diagnostic with separately profiled 2026 city terms improves 2027 agreement from 4,358 to 4,371 of 4,380 minutes; those source-fitted terms are not runtime coordinates. Source-free research code and independent checks are available.

The [noon-shape follow-up](NOON-SHAPE-TRANSFER.md) eliminates the unknown constant offset instead of choosing one: compatible annuals rise from 34/59 to 50/59, and compatible two-year city pairs from 3/12 to 10/12. These are conditional feasibility counts, not prediction accuracy. At original proxies, the hybrid gains only two exact noons overall and loses two in 2027. Isolated solar-output diagnostics still produce larger calendar errors; no runtime formula changes.
