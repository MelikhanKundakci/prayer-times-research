...
The separately published [Awqat Salah API landing page](https://awqatsalah.diyanet.gov.tr/index.html) says developers must submit a request form to receive access and lists place endpoints that return country/state/city hierarchies and a `CityDetail` by city ID. Its linked [API guide](https://awqatsalah.diyanet.gov.tr/files/56d83ac4-f7f5-4f6e-9b9e-b1ffeebf1b6a.pdf), pp. 7–9, documents place records as `id`, `code` and `name`; the sample `CityDetail` on p. 9 adds Qibla angle/distance and city/country names, but no latitude, longitude, elevation, boundary, or GPS-selection rule. The guide explains JWT authorization for requests, while the current landing page describes account provisioning and request quotas. These materials document retrieval of already named place records and prayer times; they do not publish an offline GPS-to-place assignment or calculation-point catalogue. The public API documentation therefore does not close the location gap, and no API call, city inventory, or runtime dependency was added by this audit.
