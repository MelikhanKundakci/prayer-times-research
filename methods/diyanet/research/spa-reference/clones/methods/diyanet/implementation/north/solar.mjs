...
export function dailySolar(date, latitude, longitude) {
  ...
  const s = solarCoordinatesUSNO(epoch / DAY + 2440587.5);
...
export function anchoredSolar(date, latitude, longitude, formatter) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    solar = dailySolar(new Date(carrier).toISOString().slice(0, 10), latitude, longitude);
