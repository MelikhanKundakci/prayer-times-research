// Ordering adapter for the explicit civil-row candidate; all computed instants are preserved.
import { calculateNorthernCivilRow as calculateRawNorthernCivilRow } from './model.mjs';
...
export function calculateNorthernCivilRow(options) {
  ...
  const result = calculateRawNorthernCivilRow(options);
  ...
  result.profile.frozenCalculationVersion = result.profile.version;
  result.profile.version = '0.1.1-civil-row-quality-review';
  result.qualityReview = { version: '0.1.1', unchangedEventInstants: true, checkedPairs, unavailablePairs,
    ...
    scope: 'Adjacent events within each returned civil-date row; not a cross-year or source-date certification.',
