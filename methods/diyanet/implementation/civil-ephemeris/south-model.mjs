...
function transitAt(carrier, longitude, variant, ephemerisEpoch = carrier) {
  if (variant === 'usno-daily-utc0') return carrier + (12 - longitude / 15 - sun(ephemerisEpoch).equationOfTimeHours) * HOUR;
...
function cycle(input, epoch, formatter) {
  ...
  for (let attempt = 0; attempt < 4; attempt++) {
    const transit = transitAt(carrier, input.longitude, input.variant, epoch);
...
function fixedEvents(input, carrier, transit) {
  const coordinates = sun(dateEpoch(input.date)), phi = input.latitude * RAD, delta = coordinates.declination * RAD;
