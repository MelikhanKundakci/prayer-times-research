...
import { seasonalTwilight } from './seasonal.mjs';
...
export function calculateNorthernCalendar(options) {
  ...
  // Preserve the V5 calculation-date solstice carrier, rather than silently
  // changing it to the civil label at the date line. Both are disclosed below.
  const solsticeIndex = observations.findIndex(d => d.date.endsWith('-06-21'));
  ...
  const season = seasonalTwilight(rows, dates, PARAMETERS.fajrFactor, '-06-21');
